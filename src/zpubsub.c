...
@discuss
    ...
    It allows publishing of a "topic", being a message of
...
s_sub_actor (zsock_t *pipe, void *args)
{
	...
	zsock_send (sub_beacon, "si", "CONFIGURE", 55666);
...
zpubsub_new (int domain, const char* partition, zpubsub_serialize_filter_fn *serialize_fn, zpubsub_deserialize_filter_fn *deserialize_fn)
{
    ...
    zsock_send (self->pub_beacon, "si", "CONFIGURE", 55666);
    zsock_send (self->pub_beacon, "sbi", "PUBLISH", NULL, 0, 5000);
