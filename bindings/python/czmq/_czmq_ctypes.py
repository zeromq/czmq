...
class zudp_t(Structure):
    pass # Empty - only for type checking
zudp_p = POINTER(zudp_t)
...
# zudp
lib.zudp_new.restype = socket_p
lib.zudp_new.argtypes = [c_int, c_bool]
lib.zudp_destroy.restype = None
lib.zudp_destroy.argtypes = [POINTER(zudp_p)]
lib.zudp_sendto.restype = c_int
lib.zudp_sendto.argtypes = [zudp_p, zframe_p, c_char_p, c_int]
lib.zudp_recv.restype = zframe_p
lib.zudp_recv.argtypes = [zudp_p, c_char_p, c_int]
lib.zudp_bind.restype = c_int
lib.zudp_bind.argtypes = [zudp_p, c_char_p, c_int]
lib.zudp_fd.restype = c_int
lib.zudp_fd.argtypes = [zudp_p]
lib.zudp_error.restype = None
lib.zudp_error.argtypes = [c_char_p]
lib.zudp_test.restype = None
lib.zudp_test.argtypes = [c_bool]
...
you need to reuse the socket's address.
        """
        if len(args) == 2 and type(args[0]) is c_void_p and isinstance(args[1], bool):
            self._as_parameter_ = cast(args[0], zudp_p) # Conversion from raw type to binding
            self.allow_destruct = args[1] # This is a 'fresh' value, owned by us
        elif len(args) == 2 and type(args[0]) is zudp_p and isinstance(args[1], bool):
            self._as_parameter_ = args[0] # Conversion from raw type to binding
            self.allow_destruct = args[1] # This is a 'fresh' value, owned by us
        else:
            assert(len(args) == 2)
            self._as_parameter_ = lib.zudp_new(args[0], args[1]) # Creation of new raw type
            self.allow_destruct = True
    ...
    def __del__(self):
        ...
        """
        if self.allow_destruct:
            lib.zudp_destroy(byref(self._as_parameter_))
...
disappeared (happens easily with WiFi))
        """
        return lib.zudp_sendto(self._as_parameter_, frame, address, port)
...
NI_MAXHOST if it's enabled. Returns NULL when failing to get peer address.
        """
        return Zframe(lib.zudp_recv(self._as_parameter_, peername, peerlen), False)
...
On failure, returns -1.
        """
        return lib.zudp_bind(self._as_parameter_, address, port)

    def fd(self):
        """
        Return the socket's filedescriptor
        """
        return lib.zudp_fd(self._as_parameter_)
